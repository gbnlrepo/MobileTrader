<script src="administration/jlib/normarizr.js"></script>
<script src="administration/jlib/menulib.js"></script>


<div id="body_general">
<div id="accounttile">Start Session
<span id="close"><img src="administration/jlib/cancel_icon.png"  /></span>
</div>

<p>&nbsp;</p>
<p>&nbsp;</p>
<p>&nbsp;</p>
<p>&nbsp;</p>


<div style="margin-left:30px;">
<div>
<table width="394" border="1">
            <tr>
              <td width="145"><div id="inputName">Branch:</div></td>
              <td width="233"><div id="formInputs">
                <select name="select">
                  <option value=""></option>
                </select>
              </div></td>
            </tr>
            <tr>
              <td><div id="inputName">Session Status:</div></td>
              <td><div id="formInputs">
                <select name="select">
                  <option value=""></option>
                </select>
              </div></td>
            </tr>
            <tr>
              <td><div id="inputName">New Session Date:</div></td>
              <td><div id="formInputs">
                <input type="text" value=""/>
              </div></td>
            </tr>
			  <tr>
              <td colspan="2">
			  <table width="354" border="0" cellpadding="0" cellspacing="0" style="width:350px;" >
        <tr>
          <td width="354"><div id="inputName" style="text-align:left">
            <input name="Fees&amp;Charges3" type="checkbox" value="Fees &amp; Charges" />
            All Branches </div></td>
          </tr>
      </table></td>
              </tr>
          </table>
<div id="butoss" style=" margin-top:10px;"> <input type="submit" value="Ok" /> <input type="submit" value="Close"/></div>
</div>

</div>







